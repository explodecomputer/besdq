...
def main():
    ...
    parser.add_argument('--snp-chr', required=True,
                        help='SNP chromosome')
    ...
    parser.add_argument('--probe-chr', required=True,
    ...
    # Validate and convert SNP position arguments
    ...
    if args.from_snp_bp is not None:
        ...
        print("Error: Must specify either --from-snp-kb or --from-snp-bp", file=sys.stderr)
        sys.exit(1)
    ...
    if args.to_snp_bp is not None:
        ...
        print("Error: Must specify either --to-snp-kb or --to-snp-bp", file=sys.stderr)
        sys.exit(1)
    ...
    if args.from_probe_bp is not None:
        ...
        print("Error: Must specify either --from-probe-kb or --from-probe-bp", file=sys.stderr)
        sys.exit(1)
    ...
    if args.to_probe_bp is not None:
        ...
        print("Error: Must specify either --to-probe-kb or --to-probe-bp", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        print(f"\nQuerying cis-window:")
        print(f"  SNP:   {args.snp_chr}:{snp_start_kb*1000:.0f}-{snp_end_kb*1000:.0f} bp")
        print(f"  Probe: {args.probe_chr}:{probe_start_kb*1000:.0f}-{probe_end_kb*1000:.0f} bp")
