...
Analysis metadata represents sample size with two independent fields rather than one expanding mode enum. Kind distinguishes participant count, case/control counts, effective N, and unknown values; scope distinguishes Analysis-wide values, per-variant values, and absence, supporting every required combination without conflating what a number means with where it varies.
